...
pub mod payment_escrow {
    ...
    /// Initialize the payer balance with a starting amount (default: 100 tokens)
    pub fn initialize(ctx: Context<Initialize>, initial_balance: Option<u64>) -> Result<()> {
        let acc = &mut ctx.accounts.balance;
        acc.balance = initial_balance.unwrap_or(100);
        Ok(())
    }

    /// Delegate the balance for ephemeral rollup execution
    ...
    /// Transfer an amount of tokens from the balance (core payment logic)
